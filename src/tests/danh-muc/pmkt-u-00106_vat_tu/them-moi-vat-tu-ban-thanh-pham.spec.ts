...
test.describe('PMKT-U-00106 - Thêm mới Vật tư Bán thành phẩm TC1484-TC1809', () => {
  ...
  test('TC_PMKT-U-00106-1755 - xác nhận sử dụng Đơn vị tính quy đổi Ngừng hoạt động', async ({ vatTuPage }) => {
    ...
    test.skip(!inactive, 'DB không có Đơn vị tính Ngừng hoạt động'); if (!inactive) return;
    await vatTuPage.openFirstConversionUnitDropdown(); await vatTuPage.searchFirstConversionUnit(inactive.code);
    test.skip(!(await vatTuPage.conversionUnitOptions().filter({ hasText: inactive.label }).isVisible()), 'BLOCK bởi TC1754: combogrid không hiển thị Đơn vị tính Ngừng hoạt động');
  ...
  test('TC_PMKT-U-00106-1762 - Up Down di chuyển dòng Đơn vị tính quy đổi', async ({ vatTuPage }) => {
    await prepareGoodsConversionGrid(vatTuPage, 'Bán thành phẩm'); await vatTuPage.openFirstConversionUnitDropdown(); const before = await vatTuPage.activeConversionUnitLabel();
  ...
  test('TC_PMKT-U-00106-1784 - xóa dòng quy đổi cuối cùng rồi Lưu và kiểm tra DB', async ({ vatTuPage, db }, testInfo) => {
    ...
    await vatTuPage.openMaterialTypePopup(); await vatTuPage.selectMaterialType('Bán thành phẩm');
    await recordMissingSpecialGoodsTypeBug(vatTuPage, 'Bán thành phẩm');
  ...
  test('TC_PMKT-U-00106-1809 - tự động trim khoảng trắng đầu cuối khi lưu', async () => {
    // Chuẩn bị dữ liệu: Case bị chặn bởi mismatch form Bán thành phẩm đã được xác nhận tại TC1484.
    test.skip(true, 'BLOCK bởi TC1484: form Bán thành phẩm thiếu trường Loại hàng hóa đặc trưng');
  });
